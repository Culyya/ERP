...
const RolePage: React.FC = () => {
  ...
  const [newRoleName, setNewRole] = useState<string>("");
  ...
  const totalPages = Math.ceil(filteredRole.length / rolePerPage);
  ...
  const handleCreateRole = async () => {
    ...
    try {
      ...
      Swal.fire("Error", "Failed to create the role.", "error");
    }
  };
          ...
          {isLoading ? (
            ...
            <>
              <table className="table table-hover">
                ...
                <tbody>
                  {paginatedRole.map((role, index) => (
                    <tr key={role.roleId}>
                      <td>{(currentPage - 1) * rolePerPage + index + 1}</td>
